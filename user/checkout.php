...
$stmtUser = $pdo->prepare("
    SELECT full_name, phone, address, point
    FROM users
    WHERE id = ?
");
$stmtUser->execute([$id]);
        ...
        <form action="../user/action_checkout/process_checkout.php" method="POST">
                ...
                <div class="point-box">
                    ...
                    <div class="point-use-row">
                        <input 
                            ...
                            id="usePointsInput"
                            min="0"
                            max="<?= $max_usable_points ?>"
                            value="0"
                            placeholder="Nhập số điểm muốn dùng"
                            data-total="<?= (float)$total ?>"
                            data-point-value="<?= (int)$point_value ?>"
                            data-max-points="<?= (int)$max_usable_points ?>"
                        >
            ...
            <div class="checkout-section">
                <p>
                    ...
                    Tổng tiền tạm tính:
                    <b id="subtotalText"><?= number_format($total, 0, ',', '.') ?>₫</b>
                </p>

                <p>
                    <i class="fa-solid fa-tag"></i>
                    Bạn đã được giảm:
                    <b id="pointDiscountText" style="color: var(--success);">
                        -0₫
                    </b>
                </p>
                ...
                <p>
                    ...
                    Tổng thanh toán:
                    <b id="finalTotalText" style="color: var(--danger);">
...
<?php else: ?>
    <div class="empty-cart-container">
        <h2>
            <i class="fa-solid fa-cart-shopping"></i>
            Oppss, bạn chưa có sản phẩm để thanh toán.
        </h2>
